    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="策略名称">
          <el-input v-model="listQuery.filter1" placeholder="请输入策略名称" />
        </el-form-item>
        <el-form-item label="数据输入类型">
          <el-input v-model="listQuery.filter2" placeholder="请输入数据输入类型" />
        </el-form-item>
        <el-form-item label="数据输入地址">
          <el-input v-model="listQuery.filter3" placeholder="请输入数据输入地址" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="策略名称" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="数据输入类型" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="数据输入地址" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="数据输出类型" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="启用状态" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="告警状态" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code6 }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" fixed="right" width="320">
          <template slot-scope="{ row }">
            <el-button type="primary" size="mini">策略下发</el-button>
