        ...
        <div>
            ...
            <el-form :inline="true" :model="listQuery" class="demo-form-inline">
                <el-form-item label="服务名称">
                    <el-input v-model="listQuery.filter" placeholder="请输入服务名称"></el-input>
                </el-form-item>
                <el-form-item label="服务别名">
                    <el-input v-model="listQuery.filter" placeholder="请输入服务别名"></el-input>
                </el-form-item>
                <el-form-item label="所属网关">
                    <el-input v-model="listQuery.filter" placeholder="请输入所属网关"></el-input>
                </el-form-item>
                <el-form-item>
            ...
            <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row
                ...
                <el-table-column label="规则名称" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="应用设备" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="响应设备" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="执行命令" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="检测规则条件" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="响应发射数据" prop="type3" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
