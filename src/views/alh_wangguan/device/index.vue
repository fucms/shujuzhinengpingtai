        ...
        <div>
            ...
            <el-form :inline="true" :model="listQuery" class="demo-form-inline">
                ...
                <el-form-item label="设备服务">
                    <el-input v-model="listQuery.filter" placeholder="请输入设备服务"></el-input>
                </el-form-item>
                <el-form-item label="设备配置">
                    <el-input v-model="listQuery.filter" placeholder="请输入设备配置"></el-input>
                </el-form-item>
                <el-form-item>
            ...
            <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row
                ...
                <el-table-column label="设备服务" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="设备配置" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="连接协议" prop="code" align="center">
                    <template slot-scope="{ row }">
                        <span>{{ row.code }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="管理状态" prop="type" align="center">
                    <template slot-scope="{ row }">
                        <el-tag :type="row.type == 1 ? 'primary' : 'danger'">{{ row.type == 1 ? '启用' : '关闭' }}</el-tag>
                    </template>
                </el-table-column>
